---
name: git-add-push
description: Stage changes, commit with a conventional commit prefix (feat:, fix:, chore:, docs:, refactor:, test:, style:, perf:), and push to the current tracking remote. Use whenever the user says "commit and push", "push my changes", "save and push", "add commit push", "ship this", or wants to send their current work to the remote. Also triggers on "stage and commit" or any request to persist local work upstream.
---
...
### 5. Push to the current tracking remote

Resolve the tracking remote for the current branch:

```bash
git rev-parse --abbrev-ref --symbolic-full-name @{u} 2>/dev/null
```

If a tracking remote is configured (e.g. `origin/master`), push to it:

```bash
git push
```

If no upstream is set yet, push to `origin` and set the tracking reference:

```bash
git push -u origin HEAD
```

If the remote rejects the push (non-fast-forward), report it to the user and stop — do not force-push without explicit instruction.
